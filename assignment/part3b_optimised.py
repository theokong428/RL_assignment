# -*- coding: utf-8 -*-
"""
part3b_optimised.py — Part 3b: Optimised Stochastic MECWLP (z=continuous)
优化版随机MECWLP模型

与 part3_stochastic.py 相同的两阶段随机规划结构，
但 z 改为连续变量 [0,1]（由 z=Σy 自动保证整数性），减少整数变量数。

独立运行: python part3b_optimised.py
"""
...
_tee = Tee("part3b_output.txt", sys.stdout)
sys.stdout = _tee
...
# Build and solve
# =============================================================================
log(f"Part 3b: Optimised Stochastic MECWLP — {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
log(f"Scenarios: {NUM_SCENARIOS}")
log(f"Optimisation: z as continuous [0,1] (integrality implied by z=Σy)")
log(f"\n{'='*60}")
log("Part 3b: Optimised Stochastic MECWLP (z=continuous)")
log(f"{'='*60}")

prob = xp.problem("Part3b_Optimised_Stochastic_MECWLP")
xp.setOutputEnabled(True)

# --- First-stage variables ---
y = {(j, t): xp.var(vartype=xp.binary, name=f"y_{j}_{t}")
     for j in Candidates for t in Times}
# z as continuous (optimisation)
...
log(f"  Binary: {len(y)}, Continuous: {len(z) + len(x) + len(w)}")
